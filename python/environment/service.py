...
def _compact_game_structure_for_record(game):
    ...
    for node_id, node in nodes.items():
        ...
        if not node.get(_SHANTEN_CACHE_FIELD):
            node.pop(_SHANTEN_CACHE_FIELD, None)
...
def _hydrate_game_structure_from_record(game, format_version):
    if format_version < _RECORD_FORMAT_VERSION:
        return game
    nodes = game.get("nodes") if isinstance(game, dict) else None
    if not isinstance(nodes, dict):
        return game
    ...
    for node_id, node in nodes.items():
        ...
        node["type"] = "root" if node_id == root_id else "action"
...
def _serialize_game_record_from_parts(game_copy, state_copy):
    ...
    return {
        "formatVersion": _RECORD_FORMAT_VERSION,
        "savedAt": now_iso(),
        "state": {
...
def load_game_record(record):
    ...
    _hydrate_game_structure_from_record(game, format_version)
