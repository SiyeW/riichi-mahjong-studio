...
from rms_backend.engine_runtime import (
...
class EngineRuntimeTest(unittest.TestCase):
    ...
    def test_process_stop_and_cache_invalidation_share_initialization_lock(self):
        for operation in ('restart', 'shutdown'):
            with self.subTest(operation=operation):
                with mock.patch('rms_backend.engine_runtime.EngineProcessClient') as client_type:
    ...
    def test_profile_runtime_initializes_all_assigned_outputs_once(self):
        ...
        with mock.patch("rms_backend.engine_runtime.EngineProcessClient", FakeProcessClient):
            runtime = EngineProfileRuntime(**self._runtime_specification())
