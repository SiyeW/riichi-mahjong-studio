...
- 支持立直麻将引擎协议 2.2，可显示对手向听、手牌构成、宝牌量和打点，牌张铳率、对手与牌山枚数，以及本局走向、本局收支、终局顺位和终局点数。
- 分析与控制台改为可停靠板块，可以独立显示、隐藏、拖动和调整大小，布局会自动保存。
...
- リーチ麻雀エンジンプロトコル 2.2 に対応しました。対戦相手のシャンテン数、手牌構成、ドラ枚数、打点、牌ごとの放銃率、対戦相手と牌山の残り枚数、局の展開と収支、終局時の順位と点数を表示できます。
- 解析パネルとコンソールを自由に配置できるようになりました。パネルごとに表示、非表示、移動、サイズ変更ができ、レイアウトは自動で保存されます。
...
- Add support for Riichi Engine Protocol 2.2, including opponent shanten, concealed tiles, dora and hand value; tile deal-in probabilities; opponent and wall tile counts; round outcomes and score changes; and final placements and scores.
- Make the analysis panels and console dockable. Each panel can be shown, hidden, moved, and resized independently, and the layout is saved automatically.
