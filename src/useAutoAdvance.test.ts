...
function state(overrides: Record<string, unknown> = {}) {
  return {
    ...
    readOnly: false,
    table: { phase: 'discard', currentActor: 1 } as TrainerGameView['table'],
    ...overrides,
  }
}
...
test('auto advance stops for non-play, read-only, review, and terminal states', () => {
  ...
  assert.equal(resolveAutoAdvanceDelay(state({ table: { phase: 'round_result' } as TrainerGameView['table'] })), null)
})
...
test('auto advance preserves immediate AI and controlled draw timing', () => {
  assert.equal(resolveAutoAdvanceDelay(state({ table: { autoAdvanceMode: 'ai_think' } as TrainerGameView['table'] })), 0)
  assert.equal(resolveAutoAdvanceDelay(state({ table: { phase: 'draw_or_discard', currentActor: 0 } as TrainerGameView['table'] })), 30)
  assert.equal(resolveAutoAdvanceDelay(state({ table: { riichiDiscardState: 'ankan_choice' } as TrainerGameView['table'] })), 6000)
})
...
test('legal choices block automatic progress except a completed prefetch', () => {
  ...
  assert.equal(resolveAutoAdvanceDelay(state({
    legalActionCount: 1,
    table: { phase: 'reach_declaration' } as TrainerGameView['table'],
  })), null)
})

test('reaction windows respect the larger configured or engine delay', () => {
  assert.equal(resolveAutoAdvanceDelay(state({
    table: { phase: 'reaction', reactionWindow: { thinkingTimeS: 1.25 } } as TrainerGameView['table'],
  })), 1250)
  assert.equal(resolveAutoAdvanceDelay(state({
    defaultDelayMs: 1500,
    table: { phase: 'reaction', reactionWindow: { thinkingTimeS: 1.25 } } as TrainerGameView['table'],
